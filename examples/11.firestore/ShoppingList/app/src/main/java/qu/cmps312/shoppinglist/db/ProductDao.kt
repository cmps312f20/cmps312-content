...
interface ProductDao {
    ...
    @Query("select p.id, p.name, p.icon, p.categoryId, c.name category from Product p join Category c on p.categoryId = c.id where p.categoryId = :categoryId order by p.name")
